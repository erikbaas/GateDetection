...
for i in range(len(images)):
    ...
    imgUint = (live_image * 255).astype(np.uint8)
    imgContrast = cv2.addWeighted(imgUint, 1, imgUint, 0, 0)
    imgGaussian = cv2.GaussianBlur(imgContrast, (3, 3), 1)
    imgBilateral = cv2.bilateralFilter(imgGaussian, 3, 75, 75)
    ...
    chessCoordinates = cv2.findChessboardCorners(imgGray, patternSize=(8,8))
    print("The chessboard coordinates (if found) are: ", chessCoordinates)

    # Displaying the results side to side
    StackedImages = utility.stackImages(([imgGray],
                                        [imgGray]), 0.6)
